...
def make_explore_index_html(years_with_posts, tags_with_posts):
    """Generera en kombinerad utforska-sida med tabs för tags och arkiv"""
    nav_html = create_nav(active_page='explore', depth=1)
    
    # Generera tags-sektion
    tags_html = ""
    for tag in sorted(tags_with_posts.keys()):
        post_count = len(tags_with_posts[tag])
        tags_html += f'<li><a href="tags/{tag}/">{tag}</a> ({post_count})</li>\n'
    
    # Generera arkiv-sektion
    years_html = ""
    for year in sorted(years_with_posts.keys(), reverse=True):
        post_count = sum(len(posts) for posts in years_with_posts[year].values())
        years_html += f'<li><a href="archive/{year}/">{year}</a> ({post_count})</li>\n'
...
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" href="../css/style.css">
    <title>Utforska - {SITE_TITLE}</title>
</head>
<body>
    <header class="header">
        <div class="header-content">
    ...
    <div class="grid">
        <div class="card">
            <h1>Utforska</h1>
            <p>Bläddra genom inlägg via taggar eller kronologiskt arkiv.</p>
            
            <div class="tabs">
                <button class="tab-button active" onclick="showTab(event, 'tags-tab')">Taggar</button>
                <button class="tab-button" onclick="showTab(event, 'archive-tab')">Arkiv</button>
            </div>

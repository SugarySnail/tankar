...
<body>
    ...
    <div class="grid">
        ...
        <h3>Mikrobloggen</h3>
        <ul>
            <li><a href="/rss-micro.xml">Mikroblogg</a></li>
        </ul>
...
def generate_rss_feeds(posts):
    ...
    for tag in all_tags:
        ...
        create_rss_file(filtered_posts, f"rss-{slugify(tag)}.xml", tag)
    
    create_rss_file(processed_posts, "rss.xml", None)
    print("✓ RSS-feeds genererade")
...
def rss_page():
    ...
    try:
        posts = load_posts()
        return make_rss_page_html(posts)
    except Exception as e:
        print(f"Error in rss_page: {e}")
